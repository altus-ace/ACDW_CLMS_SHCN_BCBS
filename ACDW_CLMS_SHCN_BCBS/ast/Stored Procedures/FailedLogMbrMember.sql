...
CREATE PROCEDURE [ast].[FailedLogMbrMember]@DataDate DATE, @LoadDateFrmStg DATE ---- [ast].[FailedLogMbrMember]'2021-02-18','2021-02-18'
...
BEGIN TRAN
					...
					DECLARE @JobName VARCHAR(100) = 'FailedLogMbrMember';
	...
	SELECT				(Select lBusinessRuleKey From AceMetaData.[lst].[lstBusinessRules] Where lBusinessRuleKey = 3)
						,'Failed'
	...
	FROM		
						( 
									...
									WHERE		DataDate =  @DataDate --'2020-12-20' --
...
END CATCH
END						
--  select * from AceMetaData.[amd].[AceBusinessRuleLog]
