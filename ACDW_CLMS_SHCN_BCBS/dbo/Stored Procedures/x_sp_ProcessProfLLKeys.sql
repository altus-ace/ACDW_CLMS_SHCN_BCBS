...
CREATE PROCEDURE dbo.x_sp_ProcessProfLLKeys
(   
...
DECLARE @SrcTblKey			VARCHAR(50)		= '[ProfessionalClaimKey]'
...
DECLARE @UpdateJoin			VARCHAR(100)	= 'ON		a.ClaimID	= b.' +  @ClaimID + ' '
...
/***
EXEC dbo.x_sp_ProcessProfLLKeys '[adi].[Steward_BCBS_ProfessionallClaim]','01-01-2021','dbo.z_tmp_ProfLLKeyTbl'

SELECT DISTINCT SrcLoadDate, RecStatus, count(*) as CntRec FROM dbo.z_tmp_ProfLLKeyTbl GROUP BY SrcLoadDate, RecStatus ORDER BY SrcLoadDate, RecStatus
SELECT TOP 1000 * FROM dbo.z_tmp_ProfLLKeyTbl
***/
