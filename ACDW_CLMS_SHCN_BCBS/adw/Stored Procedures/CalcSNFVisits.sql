...
CREATE PROCEDURE [adw].[CalcSNFVisits]
	...
	@PrimarySvcDate_Start	DATE,
	@PrimarySvcDate_End		DATE
	)
AS    
BEGIN
    SET NOCOUNT ON;
...
INTO #tmpAllSNFMbrs
	...
	AND a.PRIMARY_SVC_DATE	>= @PrimarySvcDate_Start 
	AND a.SVC_TO_DATE			<= @PrimarySvcDate_End
...
AND   LEFT(a.Bill_Type,2) IN (21,18) 	---
AND	a.PRIMARY_SVC_DATE	>= DATEADD(MONTH, DATEDIFF(MONTH, 0, DATEADD(M, -6,@PrimarySvcDate_Start)), 0)
AND	a.SVC_TO_DATE			<= @PrimarySvcDate_End
AND	a.TOTAL_PAID_AMT		> 0
...
SELECT * INTO #tmpCTE FROM (
	...
	FROM [adw].[2020_tvf_Get_SNFContinuousVisit] (1,DATEADD(MONTH, DATEDIFF(MONTH, 0, DATEADD(M, -6,@PrimarySvcDate_Start)), 0),@PrimarySvcDate_End) m
	WHERE m.BillType NOT IN ('213','223')
	UNION ALL
	SELECT ClientMemberKey, AssocClaimID, LastR, AssocAdmDate, AssocDischDate
	FROM [adw].[2020_tvf_Get_SNFContinuousVisit] (1,DATEADD(MONTH, DATEDIFF(MONTH, 0, DATEADD(M, -6,@PrimarySvcDate_Start)), 0),@PrimarySvcDate_End) n
...
SELECT a.ClientMemberKey, a.FirstSeqClaimID, a.FirstSvcDate, a.FirstDischDate
	...
	AND b.rank = 1
WHERE a.rank = 1 
AND YEAR(b.FirstDischDate) = YEAR(@PrimarySvcDate_End)

/*** Get UnJoined Claims ***/
SELECT * INTO #tmpUnJoinedClaims 
FROM [adw].[2020_tvf_Get_SNFNonContinuousVisit] (1,DATEADD(MONTH, DATEDIFF(MONTH, 0, DATEADD(M, -6,@PrimarySvcDate_Start)), 0),@PrimarySvcDate_End) 
WHERE year(DischargeDate) = YEAR(@PrimarySvcDate_End)
...
UPDATE adw.FctInpatientVisits
		...
		AND a.EffectiveAsOfDate = @EffectiveAsOfDate
